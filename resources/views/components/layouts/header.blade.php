...
<header
    class="fixed top-0 inset-x-0 z-50 h-[72px] border-b border-zinc-200/80 bg-white/80 backdrop-blur-xl dark:border-zinc-800/80 dark:bg-zinc-950/80 transition-all duration-300">
    <div class="flex items-center justify-between w-full h-full px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-4 lg:w-64">
            ...
            <a href="{{ route('dashboard') }}"
               class="flex items-center gap-3 outline-none rounded-lg focus-visible:ring-2 focus-visible:ring-[#FF2D20]">
                <div
                    class="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-[#FF2D20] to-[#ff7a59] text-white shadow-sm shadow-[#FF2D20]/20">
        ...
        <div class="flex items-center justify-end gap-1.5 sm:gap-3 lg:w-64">
            <button type="button"
                    class="relative p-2 text-zinc-400 transition-colors rounded-lg hover:bg-zinc-100 hover:text-zinc-900 dark:hover:bg-zinc-800 dark:hover:text-white outline-none">
                ...
                <span
                    class="absolute top-2 right-2.5 w-2 h-2 rounded-full bg-[#FF2D20] border-2 border-white dark:border-zinc-950"></span>
            </button>

            <button id="theme-toggle" type="button"
                    class="p-2 text-zinc-400 transition-colors rounded-lg hover:bg-zinc-100 hover:text-zinc-900 dark:hover:bg-zinc-800 dark:hover:text-white outline-none">
            ...
            <div class="flex items-center pl-1 gap-1 sm:gap-2">
                ...
                <form method="POST" action="{{ route('logout') }}" class="m-0 p-0 inline-flex">
                    @csrf
                    <button type="submit"
                            class="group flex items-center justify-center p-2 text-zinc-400 transition-colors rounded-lg hover:bg-red-50 hover:text-[#FF2D20] dark:hover:bg-[#FF2D20]/10 dark:hover:text-[#FF2D20] focus:outline-none"
